...
void testResult(
  ConvertedTemplateResult actual,
  ConvertedTemplateResult expected,
) {
...
void main() {
  group('convertTemplateToRegex', () {
    test('should create regex template', () {
      testResult(convertTemplateToRegex(template: '{{name}}'), (
        '(.*?)',
        ['name'],
        ['name'],
      ));
      testResult(
        convertTemplateToRegex(
          template: '{{name}}',
          matchTypeOverride: MatchType.LAZY,
        ),
        ('(.*?)', ['name'], ['name']),
      );
      testResult(
        convertTemplateToRegex(
          template: '{{name}}',
          matchTypeOverride: MatchType.GREEDY,
        ),
        ('(.*)', ['name'], ['name']),
      );
      testResult(convertTemplateToRegex(template: '{{name_GRD}}'), (
        '(.*)',
        ['name_GRD'],
        ['name_GRD'],
      ));
      ...
      testResult(
        convertTemplateToRegex(
          template: '{{hello}} {{world}}',
          parameters: {'hello': 'hi', 'world': 'there'},
        ),
