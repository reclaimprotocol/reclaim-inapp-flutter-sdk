...
void printUsage() {
  print('Usage: reclaim_inapp_sdk <genkeys|create|verify> [params]');
}

void printIncorrectUsage(String command, List<String> arguments) {
  print('Usage: reclaim_inapp_sdk $command ${arguments.map((e) => '<$e>').join(' ')}');
}
...
void genkeys() {
  ...
  final output = json.encode({'JWK': keys, 'privateKeyString': privateKeyString, 'publicKeyString': publicKeyString});
  print(output);
}
