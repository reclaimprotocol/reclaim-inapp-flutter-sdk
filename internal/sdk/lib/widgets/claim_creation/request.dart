...
import 'package:reclaim_flutter_sdk/utils/url.dart' as url_utils;
...
class ClaimCreationRequest {
  ...
  Map<String, dynamic> get httpParams =>
      _getHttpParams(extractedData, additionalClientOptions);

  Map<String, dynamic> get secretParams => _getSecretParams(extractedData);

  String get httpProviderId => providerData.httpProviderId ?? '';
  ...
  factory ClaimCreationRequest({
    ...
    final String providerUrl =
        requestData.urlType == UrlType.TEMPLATE
            ? (requestData.url ?? '')
            : proofData['url'];

    logger.info({
      'provider_url_type': requestData.urlType,
    ...
    return ClaimCreationRequest._(
      ...
      witnessParams: {
        ...witnessParams,
        if (geoLocation != null &&
            isDynamicGeoLocation(providerData.geoLocation))
          '${providerData.geoLocation?.replaceAll('{', '').replaceAll('}', '')}':
              geoLocation,
      },
      ...
      extractedData: ExtractedData(
        ...
        requestBody:
            (isRequestFromProviderScript
                ...
                : (requestData.bodySniff?.template ??
                    proofData['requestBody'])) ??
            '',
  ...
  static Map<String, String> extractUrlTemplateParams(
    ...
    final List<String?> urlParamValues =
        urlMatch
            ?.groups(List<int>.generate(urlParamKeys.length, (i) => i + 1))
            .toList() ??
        [];
...
Map<String, dynamic> _getHttpParams(
  ExtractedData data,
  Map<String, dynamic> additionalClientOptions,
) {
  ...
  return {
    ...
    'responseRedactions':
        data.responseRedactions.map((e) => e.toJson()).toList(),
