...
import 'package:reclaim_flutter_sdk/utils/chains/me_chain.dart';
import 'package:reclaim_flutter_sdk/utils/params.dart';
...
class ClaimStatus {
  ...
  ClaimRequestIdentifier get requestIdentifier =>
      request.requestData.requestIdentifier;
    ...
    switch (infoName) {
      ...
      case 'generating-zk-proofs':
        try {
          ...
          logging
              .child('ClaimStatus.currentProgress')
              .severe('Failed to compute progress', e, s);
          return null;
        }
  ...
  double get _currentProgress {
    final stepProgress =
        _currentStepProgress ?? previousStatus?._currentStepProgress ?? 0.0;
    final attestorProgress = attestorLoadingProgress;

    return (stepProgress * 0.9) + (attestorProgress * 0.1);
  }

  /// The progress of the proof computation.
  /// If this is null, that means proof computation hasn't started
  double get progress {
    final effectiveProgress = _currentProgress;
    assert(() {
...
class ClaimCreationControllerState {
  final ClaimCreationUIDelegate? delegate;
  ...
  final ReclaimVerificationProviderException? providerError;
  ...
  double? get progress {
    ...
    return progressSum / claimsByRequest.length;
  }
  ...
  bool isCompleted(ClaimRequestIdentifier requestIdentifier) {
    final completedRequestIdentifiers =
        finishedRequests.map((e) => e.requestIdentifier).toSet();
  ...
  ClaimCreationControllerState copyWithStatus(ClaimStatus status) {
    return copyWith(
      claimsByRequest: UnmodifiableMapView({
        ...claimsByRequest,
  ...
  ClaimCreationControllerState copyWith({
    ...
    ClaimCreationUIDelegate? delegate,
    ...
    ReclaimVerificationProviderException? providerError,
  }) {
