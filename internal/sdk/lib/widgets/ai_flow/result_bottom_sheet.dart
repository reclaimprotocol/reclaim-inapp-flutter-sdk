...
import 'package:reclaim_flutter_sdk/widgets/icon.dart';
...
class ResultBottomSheet extends StatefulWidget {
  ...
  final List<AIFlowDataReceipt> params;
  final AIFlowService aiFlowService;
  final String sessionId;
  final VoidCallback onSubmit;
  final String webviewUrl;
  final Function(List<AIFlowDataReceipt>) onUpdate;
  final Future<bool> Function(String) onContinue;
  final Future<void> Function() onSubmitManualVerification;
  final Future<void> Function() onAiDismiss;
  ...
  static Future<void> show(
    ...
    required List<AIFlowDataReceipt> params,
    required AIFlowService aiFlowService,
    required String sessionId,
    required String webviewUrl,
    required VoidCallback onSubmit,
    required Function(List<AIFlowDataReceipt>) onUpdate,
    required Future<bool> Function(String) onContinue,
    required Future<void> Function() onSubmitManualVerification,
    required Future<void> Function() onAiDismiss,
  }) {
...
class _ResultBottomSheetState extends State<ResultBottomSheet> {
  List<AIFlowDataReceipt> _params = [];
  ...
  void _submitData() async {
    ...
    try {
      final result = await widget.aiFlowService.extractData(
        widget.webviewUrl,
        widget.sessionId,
      );
      if (!mounted) return;
            ...
            _params.map((param) {
              ...
              if (param.extractedValue == null &&
                  (matchingResult.extractedValue != null ||
                      matchingResult.recommendation != null)) {
  ...
  bool _shouldAlert() {
    final allProviders = (_params.any((param) => param.extractedValue == null));
    final shouldAlert = !_isLoading && (_error != null || allProviders);
    return shouldAlert;
  }
          ...
          children: [
            ...
            if (_error == null)
              Text(
                _error != null
                    ? ''
                    : (_isLoading
                        ? 'AI is trying to find your data...'
                        : 'You are sharing:'),
            ...
            Column(
              children: [
                ...
                if (!_isLoading &&
                    _params.any((param) => param.extractedValue != null))
                  ActionButton(
                    onPressed: () {
                      if (_shouldAlert()) {
...
Map<String, String> aiParamsText(
  ...
  for (final param in params) {
    if (param.extractedValue != null) {
      result[param.name] = param.extractedValue!;
    } else {
      result[param.name] = hasError ? 'Error' : (param.recommendation ?? '');
    }
  }
  return result;
}

Map<String, double> aiInProgressParamsText(List<AIFlowDataReceipt> params) {
  final Map<String, double> result = {};
  for (final param in params) {
    if (param.extractedValue != null) {
      result[param.name] = 1.0;
    } else {
      result[param.name] = param.recommendation == null ? 0.5 : 1.0;
    }
  }
  return result;
}

Map<String, String> aiActionUrlsText(List<AIFlowDataReceipt> params) {
  final Map<String, String> result = {};
  for (final param in params) {
    result[param.name] = param.actionUrl ?? '';
  }
  return result;
}
...
class ParamsText extends StatelessWidget {
  ...
  final Map<String, double> inProgressParams;
  ...
  const ParamsText({
    super.key,
    required this.params,
    this.inProgressParams = const {},
    this.actionUrls = const {},
    required this.onContinue,
  });

  @override
  Widget build(BuildContext context) {
    final filteredParams = params.entries.where((entry) {
      final key = entry.key.toUpperCase().trim();
      // Hide entries that starts with 'REQ_' (case insensitive)
      return !key.startsWith('REQ_') && !key.startsWith('URL_');
    });

    final displayedParams = [...filteredParams].toList();

    final displayedParamsValues = List.generate(displayedParams.length, (
      index,
    ) {
      final entry = displayedParams.elementAt(index);
      final label = formatParamsLabel(entry.key);
      final isLast = index == displayedParams.length - 1;
      final value = entry.value.trim();
      final progress = inProgressParams[entry.key];

      final actionUrl = actionUrls[entry.key];
      Widget valueWidget;
