...
class TargetFontDescription {
  ...
  Future<void> installFontIfRequired() async {
    ...
    try {
      ...
      ByteData? byteData = await file_io.loadFontFromDeviceFileSystem(
        name: name,
...
Future<ByteData> _downloadFontDataFromNetwork(
  TargetFontDescription font,
) async {
  ...
  if (response.statusCode == 200) {
    ...
    unawaited(file_io.saveFontToDeviceFileSystem(
      name: font.name,
      fileHash: font.expectedFileHash,
      bytes: bytes,
    ));
...
Future<void> _loadFontInFlutter(
...
bool _isFileSecure(
  ...
  return font.expectedFileLength == actualFileLength &&
      font.expectedFileHash == actualFileHash;
}
