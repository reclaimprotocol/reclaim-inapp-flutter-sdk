...
String normalizeUrl(String url) {
  ...
  return Uri.parse(url)
      .replace(queryParameters: const {}, scheme: '')
      .removeFragment()
      .toString();
}
...
void safeLaunchUrl(String url) async {
  final logger = logging.child('utils.safeLaunchUrl');
  final uri = Uri.parse(url);
  if (await canLaunchUrl(uri)) {
    await launchUrl(uri);
  } else {
    logger.severe('The URL $url cannot be launched by the app',
        'Error: Could not launch $url', StackTrace.current);
  }
}
