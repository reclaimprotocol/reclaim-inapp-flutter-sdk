...
class AppInfo extends ReclaimOverride<AppInfo> {
  ...
  const AppInfo({required this.appName, required this.appImage, required this.isRecurring});

  factory AppInfo.fromJson(Map<String, dynamic> json) {
    return AppInfo(
      appName: json['application']['name'].toString(),
      appImage: json['application']['appImageUrl']?.toString() ?? '',
      isRecurring:
          json['application']['isRecurring'] == true ||
          json['application']['isRecurring']?.toString().toLowerCase() == 'true',
    );
  }
  ...
  @override
  AppInfo copyWith({String? appName, String? appImage, bool? isRecurring}) {
    return AppInfo(
