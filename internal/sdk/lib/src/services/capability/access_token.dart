...
// not importing logging because that may include use of dart:ui
final _logger = Logger('reclaim_inapp_sdk.capability_access_token');
...
const _issuer = String.fromEnvironment(
  ...
  defaultValue: 'https://dev.reclaimprotocol.org',
);
const _audience = String.fromEnvironment(
  'org.reclaimprotocol.inapp_sdk.CAPABILITY_ACCESS_TOKEN_AUDIENCE',
  defaultValue: 'org.reclaimprotocol.inapp_sdk',
);

class CapabilityAccessTokenException implements Exception {
  const CapabilityAccessTokenException(this.message);
...
class InvalidCapabilityAccessTokenException extends CapabilityAccessTokenException {
  const InvalidCapabilityAccessTokenException(super.message);
...
class ExpiredCapabilityAccessTokenException extends CapabilityAccessTokenException {
  const ExpiredCapabilityAccessTokenException(super.message);
...
class CapabilityAccessToken extends ReclaimOverride<CapabilityAccessToken> {
  final ES256Jws accessToken;

  final Set<String> capabilities;
  final Set<Uri> authorizedParties;
  ...
  factory CapabilityAccessToken({required ES256Jws accessToken}) {
    final nowInMs = DateTime.now().toUtc().millisecondsSinceEpoch;
    final now = (nowInMs / 1000).round();

    final payload = json.decode(utf8.decode(accessToken.payload));
    ...
    final Set<String> capabilities = scope.split(' ').toSet();
    final Set<Uri> authorizedParties = azp.split(' ').where((e) => e.isNotEmpty).map((e) => Uri.parse(e)).toSet();
    return CapabilityAccessToken._(
  ...
  @override
  ReclaimOverride<CapabilityAccessToken> copyWith({ES256Jws? accessToken}) {
    return CapabilityAccessToken(accessToken: accessToken ?? this.accessToken);
  }
  ...
  factory CapabilityAccessToken.create(
    ...
    required String sub,
    Duration expiresAfter = const Duration(days: 400),
  }) {
    final scope = capabilities.join(' ');
    ...
    final expiresAt = expiresAtDateTime.millisecondsSinceEpoch ~/ 1000;
    ...
    final payload = utf8.encode(
      json.encode({
        ...
        'sub': sub,
        'scope': scope,
        if (azp.isNotEmpty) 'azp': azp,
      }),
    );

    final privateKey = json.decode(utf8.decode(urlSafeDecode(privateKeyString))) as Map<String, dynamic>;

    final jws = ES256Jws.create(payload, privateKey);
  ...
  factory CapabilityAccessToken.import(String accessTokenString, String publicKeyString) {
    try {
      ...
      _logger.severe('Invalid Capability Access Token', e, s);
