...
Dio buildDio() {
  ...
  dio.interceptors.add(
    RetryInterceptor(
      ...
      retryEvaluator: ReclaimDioRetryEvaluator({...defaultRetryableStatuses}).evaluate,
    ),
  );
  return dio;
}
...
class ReclaimDioRetryEvaluator {
  ...
  FutureOr<bool> evaluate(DioException error, int attempt) {
    ...
    bool shouldRetry;
    if (error.type == DioExceptionType.unknown || error.toString().contains('net::ERR_TIMED_OUT')) {
      log.warning(
        {"reason": "An Unknown client error occurred", "exception": error.toString()},
        error.error,
        error.stackTrace,
      );
      shouldRetry = true;
