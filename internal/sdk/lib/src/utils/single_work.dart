...
class WorkCanceledException implements Exception {
...
class SingleWorkScope<T> {
  final List<Future<T>> _pending = [];

  int get pendingCount => _pending.length;

  void _addAsPending(Future<T> future) async {
    _pending.add(future);
    try {
      await future;
    } catch (_) {}
  ...
  Future<Completer<T>> acquireWorkCompleter() async {
    final previous = [..._pending];
    final completer = Completer<T>();

    _addAsPending(completer.future);

    for (final future in previous) {
  ...
  Future<T> runGuarded(Future<T> Function() work) async {
    final completer = await acquireWorkCompleter();

    void runWorkAndInformCompletion() async {
      try {
