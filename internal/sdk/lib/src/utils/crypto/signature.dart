...
class NistP256CurveInfo {
  ...
  Map<String, dynamic> exportPrivateKey(String kid, ECPrivateKey privateKey, ECPublicKey publicKey) {
...
class NistP256ECDSASigner {
  ...
  factory NistP256ECDSASigner.fromJwk(Map<String, dynamic> jwk) {
    ...
    return NistP256ECDSASigner(
      kid: jwk['kid'] ?? '',
      keyPair: AsymmetricKeyPair(curveInfo.importPublicKeyFromJwk(jwk), curveInfo.importPrivateKeyFromJwk(jwk)),
      curveInfo: curveInfo,
    );
  }
...
class NistP256ECDSAVerifier {
  ...
  const NistP256ECDSAVerifier({required this.kid, required this.publicKey, required this.curveInfo});
