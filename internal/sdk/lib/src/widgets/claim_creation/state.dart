...
class ClaimCreationErrorDetails {
  ...
  const ClaimCreationErrorDetails({this.error, this.stackTrace, this.reason, this.humanReadableReason});
}
...
class ClaimStatus {
  ...
  final ClaimStatus? previousStatus;
  final DateTime creationDate;
  final double attestorLoadingProgress;
  ...
  ClaimStatus createNext({
    ...
    double? attestorLoadingProgress,
  }) {
    return ClaimStatus(
      request: request,
  ...
  bool get isComputingProofs {
    ...
    final step = stepInformation;
    return step != null && step.isNotEmpty;
  }

  bool get isIdle {
    return proofs == null && _currentStepProgress == null && error == null;
  }

  double? get _currentStepProgress {
    final step = stepInformation?['step'];
    if (step == null) return null;
    if (isMeChainStep(step['type'])) {
  ...
  double get _currentProgress {
    final stepProgress = _currentStepProgress ?? previousStatus?._currentStepProgress ?? 0.0;
    final attestorProgress = attestorLoadingProgress;

    return (stepProgress * 0.8) + (attestorProgress * 0.2);
  }
  ...
  ClaimStatus copyWith({
    ...
    return ClaimStatus(
      ...
      creationDate: creationDate,
      attestorLoadingProgress: attestorLoadingProgress ?? this.attestorLoadingProgress,
      performanceReports: performanceReports ?? this.performanceReports,
    );
  }
}

typedef ClaimRequestIdentifier = String;

@immutable
class ClaimCreationControllerState {
  final ClaimCreationUIScopeState? delegate;
  final Map<ClaimRequestIdentifier, ClaimStatus> claimsByRequest;
  final Object? publicData;
  final HttpProvider? httpProvider;
  final ClaimCreationStatus status;
  ...
  const ClaimCreationControllerState({
    ...
    this.delegate,
    this.publicData,
    this.claimsByRequest = const {},
    this.status = ClaimCreationStatus.ready,
  ...
  int get _maxExpectedClaims {
    return math.max(
      ...
      1,
      httpProvider?.requestData.length ?? 0,
    );
  }

  Iterable<ClaimStatus> get claims =>
      claimsByRequest.values.toList()..sort((a, b) => a.creationDate.compareTo(b.creationDate));

  bool get isIdle => claimsByRequest.isEmpty;

  bool get isIncomplete {
    if (canExpectManyClaims) return true;
    return _maxExpectedClaims > claimsByRequest.length;
  }

  bool get isWaitingForContinuation {
    return !isComputingProofs && isIncomplete;
  }
  ...
  bool get hasVerifiedAtleastOneClaim {
    return claimsByRequest.values.any((status) => status.proofs != null);
  }

  bool get isFinished {
    return claimsByRequest.values.every((status) => status.proofs != null) && !isIncomplete;
  }

  bool get hasProviderScriptError {
  ...
  double? get progress {
    ...
    final progressSum = claimsByRequest.values.fold(0.0, (a, b) {
      return a + b.progress;
    });

    final maxRequests = math.max(_maxExpectedClaims, claimsByRequest.length);

    return progressSum / maxRequests;
  }

  Map<String, double> get paramsProgress {
    return claimsByRequest.values
            .map((status) {
              final paramNames = paramNamesFromRequestData([status.request.requestData]);
  ...
  Iterable<ClaimStatus> get finishedRequests {
    return claimsByRequest.values.where((status) => status.proofs != null);
  }

  bool isCompleted(ClaimRequestIdentifier requestIdentifier) {
    final completedRequestIdentifiers = finishedRequests.map((e) => e.requestIdentifier).toSet();
    return completedRequestIdentifiers.contains(requestIdentifier);
  }

  ClaimStatus? getCompletedRequestBy(ClaimRequestIdentifier requestIdentifier) {
    for (final request in finishedRequests) {
      if (request.requestIdentifier == requestIdentifier) {
        return request;
      }
    }
    return null;
  ...
  Iterable<ClaimStatus> get erroredRequests {
    return claimsByRequest.values.where((status) => status.error != null);
  }

  int get totalCount {
    return claimsByRequest.length;
  }

  ClaimStatus? maybeGet(ClaimRequestIdentifier requestIdentifier) {
    return claimsByRequest[requestIdentifier];
  }

  ClaimStatus get(ClaimRequestIdentifier requestIdentifier) {
    return maybeGet(requestIdentifier)!;
  }

  ClaimCreationControllerState copyWithStatus(ClaimStatus status) {
    return copyWith(claimsByRequest: UnmodifiableMapView({...claimsByRequest, status.requestIdentifier: status}));
  }

  bool get inProgressOrCompleted => isComputingProofs || isFinished;

  ClaimCreationControllerState copyWith({
