...
class WorkerManager<INPUT_TYPE, OUTPUT_TYPE> {
  ...
  Future<BackgroundWorker<INPUT_TYPE, OUTPUT_TYPE>> createWorker({
    String? debugLabel,
    MessageHandler? onMessageFromBackground,
  }) async {
    return BackgroundWorker._spawn(
      runnable: runnable,
      debugLabel: debugLabel,
      onMessageFromBackground: onMessageFromBackground,
    );
  }
}

final _logger = sdkLogger.child('BackgroundWorker');

class MessageForOwner {
  final Object? data;

  const MessageForOwner(this.data);
}
...
class BackgroundWorker<INPUT_TYPE, OUTPUT_TYPE> {
  ...
  final MessageHandler? onMessageFromBackground;

  BackgroundWorker._(this._commands, this._responses, this.debugLabel, this.onMessageFromBackground) {
  ...
  Future<OUTPUT_TYPE> executeInBackground(INPUT_TYPE input, {Duration? timeout}) async {
    ...
    if (timeout != null) {
      return await completer.future.timeout(timeout) as OUTPUT_TYPE;
    }
  ...
  static Future<BackgroundWorker<INPUT_TYPE, OUTPUT_TYPE>> _spawn<INPUT_TYPE, OUTPUT_TYPE>({
    ...
    required MessageHandler? onMessageFromBackground,
  }) async {
    ...
    bool didReceiveInitialMessage = false;
    initPort.handler = (message) {
      if (!didReceiveInitialMessage) {
        didReceiveInitialMessage = true;
    ...
    try {
      ...
      return BackgroundWorker._(sendPort, receivePort, effectiveDebugLabel, onMessageFromBackground);
    } on Object {
      initPort.close();
      rethrow;
    }
  ...
  void _handleResponsesFromIsolate(dynamic message) {
    ...
    if (message is MessageForOwner) {
      if (onMessageFromBackground != null) {
        onMessageFromBackground!(message.data);
      } else {
        _logger.warning('Received message from background worker but no handler was provided, message was: $message');
      }
      return;
    }
  ...
  static void _handleCommandsToIsolate<INPUT_TYPE, OUTPUT_TYPE>(
    ...
    receivePort.listen((message) async {
      ...
      try {
        final response = await runnable.call(
          input,
          debugLabel: debugLabel,
          receivePort: receivePort,
          sendPort: sendPort,
