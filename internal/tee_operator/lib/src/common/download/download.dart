  ...
  try {
    if (Platform.isAndroid) {
      ...
      config.cache = cupertino.URLCache.withCapacity(
        memoryCapacity: 100 * 1024 * 1024, // 100MB memory (reduced from 200MB)
        diskCapacity: 700 * 1024 * 1024, // 700MB disk (increased from 200MB for large files)
      ...
      // Note: iOS URLCache has a default per-file limit of ~5-10% of total cache
      // With 500MB disk cache, it should cache files up to ~50MB
      return cupertino.CupertinoClient.fromSessionConfiguration(config);
    }
  } catch (e, s) {
