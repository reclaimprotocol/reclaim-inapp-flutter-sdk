...
import 'utils/cancel/cancel.dart';
...
export 'types/create_claim.dart';
...
class ReclaimVerification
  ...
  final String
      secret;
  final ReclaimSessionInformation
      sessionInformation;
  final String
      context;
  final Map<
  ...
  Future<List<CreateClaimOutput>?>
    ...
    final logger =
        logging.child('ReclaimVerification.startVerification');
    initializeReclaimLogging();
    // If there's existing modal, pop it
    final currentRoute = ModalRoute.of(buildContext)
        ?.settings
        .name;
    ...
    return Future.error(
        ReclaimException.onError(result));
  }
}

class _ReclaimVerificationPage
...
class ReclaimVerificationModal
...
class ReclaimVerificationModalState
    extends State<
        ReclaimVerificationModal> {
  late String
      privateKey =
      generatePrivateKey();
  late ReclaimSessionInformation
      sessionInformation =
  ...
  void
    ...
    _onCancelledListenerRemover = widget
        .cancellable
  ...
  late final _loadingProgressNotifier =
      ValueNotifier<double>(0.01);

  void setInitProgress(
      double
          progress) {
    if (!mounted)
      return;
    _loadingProgressNotifier.value =
        progress;
  }
  ...
  void
    ...
    if (!mounted)
      return;
    final logger =
        logging.child('ReclaimVerificationModal._onCancelled');
    logger
        .info('Cancelling modal $hashCode');
    Navigator.of(context)
        .pop(const ReclaimVerificationCancelledException());
  }

  void
      _initializeSession() async {
    final logger =
        logging.child('ReclaimVerificationModal._initializeSession');
    final navigator =
        Navigator.of(context);
    try {
  ...
  Future<void>
      _validateAndCreateNewSessionIfRequired() async {
    if (sessionInformation
        .isValid)
      return;

    sessionInformation =
        await ReclaimSessionInformation.generateNew(
      providerId:
          widget.providerId,
      applicationSecret:
          widget.secret,
  ...
  AttestorAuthenticationRequest?
      _attestorAuthenticationRequest;

  Future<void>
      _fetchAttestorAuthenticationRequest(
    HttpProvider
        provider,
    ReclaimAttestorAuthenticationRequestCallback
        callback,
  ) async {
    final logger =
        logging.child(
  ...
  Future<void>
      _onFetchAttestorAuthenticationRequest() async {
    final attestorAuthRequestCallback = widget
        .verificationOptions
        ?.attestorAuthenticationRequest;
    if (attestorAuthRequestCallback !=
        null) {
      final httpProvider =
          dataProviders?.httpProvider?.firstOrNull;
      if (httpProvider !=
          null) {
  ...
  Future<HttpProvider?>
    ...
    final response =
        await ReclaimProviderService().getProviders(
      widget.appId,
      widget.providerId,
      sessionInformation.sessionId,
  ...
  Widget build(
      BuildContext
          context) {
    final providerData = dataProviders
        ?.httpProvider
        ?.firstOrNull;
    final loginUrl =
        providerData?.loginUrl ?? 'loading...';
