...
import '../src/utils/sanitize.dart';
...
import '../widgets/ai_flow/result_bottom_sheet.dart';
import '../widgets/claim_creation/claim_creation.dart';
import '../widgets/claim_creation/trigger_indicator.dart';
import '../widgets/debug_bottom_sheet.dart';
import '../widgets/recommendation_bar.dart';
...
class WebviewScreen
  ...
  final HttpProvider?
      providerData;
  final ReclaimVerificationOptions?
      verificationOptions;
...
class WebviewScreenState
  ...
  late final logger =
      logging.child('WebviewScreenState');
  ...
  late ClaimCreationController
      claimCreationController;
  late AIFlowService
      aiFlowService;
  ...
  CreateWindowAction?
      _createWindowAction;
  ClaimCreationStatus?
      oldClaimCreationState;

  Timer?
      _manualVerificationPromptTimer;
  Timer?
      _requestShareConsentPromptTimer;
  Timer?
      _aiCheckingTimer;

  final ValueNotifier<double>
      _webviewProgressNotifier =
      ValueNotifier(0.0);
  String
      _webviewUrl =
      "";
  Completer<void>?
      _websiteLoadingCompleter;
  ...
  Future<String>
    ...
    try {
      ...
      logger.severe(
          'Failed to get current referer url',
          e,
          s);
    }
    return url_util.createRefererUrl(currentUrl) ??
        _webviewUrl;
  }
  ...
  final Completer<UnmodifiableListView<UserScript>>
      _userScriptsCompleter =
  ...
  void
    ...
    SharedPreferences.getInstance()
      ...
      final cookiePersistPref =
          Flags.isCookiePersist(prefs);
      final isSingleReclaimRequestPref =
          Flags.isSingleReclaimRequest(prefs);
      idleTimeThreshold =
          Flags.getIdleTimeThreshold(prefs);
      _isWebInspectable =
          Flags.isWebInspectable(prefs);
      _isAiFlowEnabled =
          Flags.isAIFlowEnabled(prefs);
      setState(() {
    ...
    claimCreationController
        .addListener(_onClaimCreationUpdate);
    aiFlowService =
        AIFlowService(
  ...
  void
      _loadUserScripts() async {
    try {
      final userScripts =
          await UserScriptService.createUserScripts(
      ...
      _userScriptsCompleter.completeError(e,
          s);
  ...
  void
      didChangeMetrics() {
    final bottomInset = View.of(context)
        .viewInsets
        .bottom;
    bool
        isKeyboardVisible =
  ...
  bool
      isAIProvider() {
    return widget.providerData?.isAIProvider() ??
        false;
  }
  ...
  void _updateAiFlowDataReceipts(
      List<AIFlowDataReceipt>
          receipts) {
    setState(
        () {
      _aiFlowDataReceipts =
        ...
        if (param.extractedValue == null && (matchingResult.extractedValue != null)) {
  ...
  Future<void>
      submitManualVerification() async {
    final log =
        logger.child('submitManualVerification');
    log.info(
        'Submitting manual verification');
    ...
    final payload =
      ...
      parameters: _aiFlowDataReceipts
    ...
    await aiFlowService
      ...
      widget.providerData!.httpProviderId!,
      Map.fromEntries(
        _aiFlowDataReceipts.where((r) => r.extractedValue != null).map((r) => MapEntry(r.name, r.extractedValue!)),
      ),
    );
  }
  ...
  Future<bool>
      onAiContinue(String expectedPageUrl) async {
    _isAIVerificationStarted =
        false;
    return _onContinue(
        expectedPageUrl);
  ...
  void
    ...
    if (!_hasShownLoginToast) {
      ...
      final fieldNames = _aiFlowDataReceipts
          .map(
            (r) => r.name.split("_").map((word) => word[0].toUpperCase() + word.substring(1)).join(" "),
          )
          .join(", ");
  ...
  Future<void>
      _triggerAIChecking() async {
    final log =
        logger.child('_triggerAIChecking');
    if (!mounted ||
        aiCheckingLoading ||
        _isAIVerificationStarted ||
        _isKeyboardVisible ||
  ...
  _onClaimCreationUpdate() async {
    final status = claimCreationController
        .value
        .status;
    if (oldClaimCreationState !=
        status) {
      switch (status) {
  ...
  bool
      _isClosedWithException =
      false;

  void _onReclaimException(
      ReclaimException
          e) {
    if (e
        is ReclaimVerificationRequirementException) {
      // Requirement for verification could not be met. We can ignore this error.
      return;
    }
    if (_isClosedWithException)
      return;
    _isClosedWithException =
        true;
    if (mounted) {
  ...
  void _onSubmitProofs(
      Iterable<CreateClaimOutput>
          proofs) {
    final log =
        logger.child('_onCreateClaimOutput');
    log.finest(
        'Closing screen with claim output');
    // Pop every route from navigation stack repeatedly until we reach the Reclaim Webview Screen
    Navigator.of(context)
        .popUntil((route) {
      return route.settings.name ==
          reclaimInAppSDKRouteSettings.name;
    });
    // Pop Reclaim Webview Screen with Result
    Navigator.of(context)
        .pop(UnmodifiableListView(proofs));
  }
    ...
    showModalBottomSheet(
      ...
      builder:
        ...
        return StatefulBuilder(
          builder: (context, setState) => DebugBottomSheet(
            refreshPage: () {
  ...
  Future<void>
      _updateWebInspectable() async {
    final isInspectable =
        Flags.isWebInspectable(
    ...
    if (ctrl ==
      ...
      return;
    final settings =
        await ctrl.getSettings() ?? InAppWebViewSettings();
    settings.isInspectable =
        isInspectable;
    await ctrl.setSettings(
        settings: settings);
  }
  ...
  void onProofDataReceived(
    ...
    try {
      ...
      final url =
          WebUri(proofData['url']);
      final String?
          requestHash =
          proofData['matchedRequest']['requestHash'];
      assert(
        requestHash != null,
      ...
      final String
          cookieString =
          await url_util.getCookieString(url);
      ...
      if (!_useSingleRequest) {
        ...
        request = await claimCreationController.createRequestWithUpdatedProviderParams(
          proofData['response'],
          request,
        );
      }
      ...
      logger.severe(
          'Claim creation stopped due to a reclaim exception',
          e,
          s);
      _onReclaimException(e);
    } catch (e, s) {
      logger.severe(
          'Claim creation stopped due to an error',
          e,
          s);
    } finally {
  ...
  Future<void>
    ...
    try {
      ...
      final requestData =
          DataProviderRequest.fromScriptInvocation(extractData);
      if (claimCreationController.value.isCompleted(
        requestData.requestIdentifier,
      )) {
        logger.info(
      ...
      final Map<String, String>
          headers =
          (extractData['headers'] is Map ? ensureMap<String, String>(extractData['headers']) : null) ?? <String, String>{};
      final refererUrl =
          await getCurrentRefererUrl();
      headers['Referer'] =
          refererUrl;
      ...
      final geoLocation =
          await getUserLocation(extractData['geoLocation']);
  ...
  Future<GeolocationPermissionShowPromptResponse> _onGeolocationPermissionsShowPrompt(
      ...
      origin) async {
    final log =
        logger.child('_onGeolocationPermissionsShowPrompt');
    try {
      final messenger =
  ...
  void _onWebViewCreated(
    ...
    controller
      ...
      callback:
          (args) async {
        if (_isAiRecommendationDismissed)
          return;
  ...
  String?
      _nextExpectedPageUrl;

  void evaluateExpectedPageUrlOnLoadStop(
      String?
          url) async {
    final log =
        logger.child('evaluateUrlOnLoadStop');

    final expectedUrl =
        claimCreationController.getNextLocation();
    if (expectedUrl ==
        null) {
    ...
    if (!await _canContinueWithExpectedUrl(
        expectedUrl))
      return;
  ...
  void
    ...
    _nextExpectedPageUrl =
        null;
    final didContinue =
        await _onContinue(expectedPage);
  ...
  Future<bool>
      _canContinueWithExpectedUrl(String expectedPageUrl) async {
    final log =
        logger.child('WebviewScreenState._canContinueWithExpectedUrl');
    final controller =
        webViewController;
    if (controller ==
        null)
  ...
  void
    ...
    logger
        .info('Sending requests for diagnosis');
    try {
      final logs =
          requestLogs;
      if (logs == null ||
      ...
      logger.severe(
          'Failed to send requests for diagnosis',
          e,
          s);
    } finally {
      if (mounted) {
  ...
  bool
      didConsentToShareRequests =
      false;

  void
      _showRequestShareConsentPrompt() async {
    if (!mounted)
      return;
    ...
    final log =
        logger.child('_showRequestShareConsentPrompt');
    log.finer(
        'Asking for permission to dump requests now');
    late ScaffoldFeatureController<
        SnackBar,
        SnackBarClosedReason> ctrl;
    final msg =
        ScaffoldMessenger.of(context);
    ...
    void
      ...
      _sendRequestsForDiagnosisPeriodic();
      ctrl.close();
      if (claimCreationController.value.delegate?.isBottomSheetOpen ==
          true) {
        Navigator.of(context).pop();
      }
      unawaitedSequence([
  ...
  Future<void>
      _scheduleToShowRequestShareConsentPromptIfRequired() async {
    final log =
        logger.child('_showRequestShareConsentPromptIfRequired');
    final prefs =
        await SharedPreferences.getInstance();
    final canUseAiFlow =
        Flags.getCanUseAiFlow(prefs);
    if (!canUseAiFlow) {
      requestLogs =
          null;
      log.finest(
    ...
    claimCreationController
        .addListener(_showRequestShareConsentPromptIfError);
    if (!mounted)
      return;
    log.finer(
        'scheduling prompting request share consent prompt');
    _requestShareConsentPromptTimer
        ?.cancel();
  ...
  Future<bool>
      _onContinue(String expectedPageUrl) async {
    final log =
        logger.child('WebviewScreenState._onContinue');
    final controller =
        webViewController;
    if (controller ==
        null)
    ...
    final currentHost = currentUrl != null
        ? Uri.parse(currentUrl).host
        : '';
    final fullExpectedUrl = expectedPageUrl.startsWith('http')
        ? expectedPageUrl
    ...
    await controller
        .loadUrl(
      urlRequest:
          URLRequest(url: WebUri(fullExpectedUrl)),
    );
    return true;
  }
  ...
  Widget build(
    ...
    return ClaimCreationScope(
      uiDelegateOptions:
          ClaimCreationUIDelegateOptions(
                  ...
                  builder: (context, snapshot) {
                    return WebviewBar(
                      webviewUrl: (() {
                        ...
                        return widget.providerData?.loginUrl ?? 'loading...';
                      })(),
                      webviewProgress: _webviewProgressNotifier.value,
                      openDebugMenu: () => _showDebugBottomSheet(),
                      close: !widget.hideCloseButton ? () => Navigator.of(context).pop() : null,
                    );
                  },
                ),
                Expanded(
                        ...
                        builder: (context, snapshot) {
                          ...
                          return InAppWebView(
                            ...
                            onLoadStart: (controller, url) {
                              ...
                              logger.child('webview_load_event').info('Webview load start $url');
                              setState(() {
                                //
                              });
                            },
                            onProgressChanged: (controller, progress) {
                              setState(() {
                                _webviewProgressNotifier.value = progress / 100.0;
                              });
                            },
                            onLoadStop: (controller, url) async {
                              logger.child('webview_load_event').info('Webview load stop $url');
                              markWebsiteLoaded();
                              _webviewUrl = url?.toString() ?? '';
                            ...
                            initialSettings: InAppWebViewSettings(
                              ...
                              incognito: widget.providerData?.useIncognitoWebview,
                            ),
...
class _ContinueExpectedPageActionBar
    extends StatelessWidget {
  final VoidCallback
      onPressed;
  const _ContinueExpectedPageActionBar(
  ...
  Widget build(
    ...
    return ActionBar(
      onPressed:
          onPressed,
      label:
          'Tap to continue verification',
      backgroundColor:
          const Color(0xffffc636),
      foregroundColor:
          Colors.black,
    );
  }
