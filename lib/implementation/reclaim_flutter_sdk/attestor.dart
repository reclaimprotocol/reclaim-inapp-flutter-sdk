...
class Attestor {
  ...
  Future<AttestorClient>
      _createAttestorWebView() async {
    final prefs =
    ...
    final attestor =
        AttestorWebViewClient(attestorBrowserRpcUrl: effectiveUrl);
    ...
    final callback =
        _computeAttestorProof;
    if (callback !=
        null) {
      attestor.zkOperator =
          CallbackAttestorZkOperator.withComputeProof(
        callback,
      );
    }
  ...
  Future<void>
      _disposeAttestor(AttestorClient attestor) async {
    return attestor
        .dispose();
  }

  @protected
  late final _attestorPool =
      ReusableResourcePool(
    initialPoolSize:
        6,
    createResource:
        _createAttestorWebView,
    disposeResource:
        _disposeAttestor,
  );

  ReusableResourcePool<AttestorClient>
      get pool =>
          _attestorPool;
  ...
  Future<
      T> useAttestor<
          T>(
      Future<T> Function(AttestorClient)
          use) {
    return _attestorPool
        .compute(use);
  }
  ...
  Future<AttestorProcess<AttestorClaimRequest, List<AttestorClaimResponse>>>
    ...
    AttestorCreateClaimPerformanceReportCallback?
        onPerformanceReports,
  }) {
    return useAttestor(
        (attestor) async {
      VoidCallback?
          createInitProgressListener() {
        final it = attestor;
  ...
  Future<void>
      setAttestorDebugLevel(String level) {
    _level =
        level;
    return Future
        .wait(
      pool.resources.map((e) =>
          e.setAttestorDebugLevel(level).response),
    );
  }
  ...
  Future<void>
      setAttestorUrl(Uri value) async {
    if (_attestorUrl ==
        value) {
      return;
    }
  ...
  Future<void>
      setComputeAttestorProof(
    ComputeProofForAttestorCallback
        callback,
  ) async {
    if (_computeAttestorProof ==
        callback)
      return;
    _computeAttestorProof =
        callback;
  ...
  Future<String> extractJSONValueIndex(
      String
          json,
      String
          jsonPath) {
  ...
  Future<String> extractHtmlElement(
      String
          html,
      String
          xpathExpression) {
