...
import '../injections/misc.dart';
import '../logging/logging.dart';
import '../src/web_scripts/web_scripts.dart';
import '../webview_utils.dart';

class UserScriptService {
  static final logger =
      logging.child('UserScriptService');

  static Future<UnmodifiableListView<UserScript>>
      createUserScripts({
    ...
    try {
      ...
      if (providerData.customInjection != null &&
          providerData.customInjection!.isNotEmpty) {
        scripts.add(
          UserScript(
            source: providerData.customInjection!,
            injectionTime: UserScriptInjectionTime.AT_DOCUMENT_START,
          ),
        );
      }
      ...
      scripts.add(
        UserScript(
          source: SUPPORT_RN_CUSTOM_INJECTIONS,
          injectionTime: UserScriptInjectionTime.AT_DOCUMENT_START,
        ),
      );
      ...
      scripts.add(
        UserScript(
          source: loginButtonHeuristicsInjection(),
          injectionTime: UserScriptInjectionTime.AT_DOCUMENT_START,
        ),
      );
  ...
  static String
    ...
    try {
      final payload = (providerData?.toJson() ??
          <String, dynamic>{})
        ..remove('customInjection')
        ..addEntries([
          MapEntry('parameters', parameters)
        ]);
      ...
      logger.severe(
          'Error creating payload data script',
          e,
          s);
      rethrow;
    }
  ...
  static String
    ...
    try {
      final injectionRequests =
          providerData?.requestData.map((e) {
        return InjectionRequest(
          urlRegex: convertTemplateToRegex(
            template: e.url ?? '',
      ...
      logger.severe(
          'Error creating intercepter injection',
          e,
          s);
      rethrow;
    }
  ...
  static void
    ...
    try {
      ...
      List<String?>
          urlParamValues =
          match.groups(List<int>.generate(urlParamKeys.length, (i) => i + 1)).toList();

      urlParamKeys.asMap().forEach((key,
          value) {
      ...
      logger.severe(
          'Error extracting URL template params',
          e,
          s);
      rethrow;
    }
