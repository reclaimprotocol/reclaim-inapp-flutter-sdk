...
final _networkLogsClient =
    buildDio();

class NetworkLogsService {
  final log =
      logging.child('NetworkLogsService');
  static final _chunkCount =
      <String,
          int>{};
  Future<bool>
      addToQueue(
    ...
    final chunkId =
        _chunkCount[sessionId] ?? 1;
    _chunkCount[sessionId] =
        chunkId + 1;
    try {
      final data =
          json.encode({
...
List<
  ...
  if (m <=
      0) {
    throw ArgumentError(
        'Number of chunks (m) must be positive.');
  }
