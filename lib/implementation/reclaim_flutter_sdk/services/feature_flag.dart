...
class FeatureFlagService {
  ...
  static Future<ReclaimFeatureFlagData>
      getFeatureFlagsWithOverrides(SessionIdentity identity) async {
    final logger =
        logging.child('FeatureFlags.getFeatureFlagsWithOverrides');

    final featureFlagsOverrides =
        ReclaimOverrides.featureFlag;
    ...
    final attestorBrowserRpcUrlOverride =
        featureFlagsOverrides?.attestorBrowserRpcUrl;

    final featureFlagNames =
        [
      if (featureFlagsOverrides?.idleTimeThresholdForManualVerificationTrigger ==
          null)
        'idleTimeThresholdForManualVerificationTrigger',
      if (featureFlagsOverrides?.sessionTimeoutForManualVerificationTrigger ==
          null)
        'sessionTimeoutForManualVerificationTrigger',
      if (attestorBrowserRpcUrlOverride == null ||
          attestorBrowserRpcUrlOverride.isEmpty)
        'attestorBrowserRpcUrl',
      if (featureFlagsOverrides?.isAIFlowEnabled ==
          null)
        'isAIFlowEnabled',
      if (featureFlagsOverrides?.canUseAiFlow ==
          null)
        'canUseAiFlow'
    ];
    ...
    logger
        .info({
      'flags_to_fetch_from_server':
          featureFlagNames,
    });

    final flags = await getFeatureFlagsRaw(
        identity,
        featureFlagNames);
    ...
    final effectiveFlags =
      ...
      idleTimeThresholdForManualVerificationTrigger:
          featureFlagsOverrides?.idleTimeThresholdForManualVerificationTrigger ?? flags['idleTimeThresholdForManualVerificationTrigger'],
      sessionTimeoutForManualVerificationTrigger:
          featureFlagsOverrides?.sessionTimeoutForManualVerificationTrigger ?? flags['sessionTimeoutForManualVerificationTrigger'],
      ...
      isAIFlowEnabled: featureFlagsOverrides?.isAIFlowEnabled ??
          flags['isAIFlowEnabled'] ??
          false,
      canUseAiFlow: featureFlagsOverrides?.canUseAiFlow ??
          flags['canUseAiFlow'] ??
          false,
    );
  ...
  static Future<Map<String, dynamic>>
    ...
    final String
        privateKey =
        await storage.getData('ReclaimOwnerPrivateKey');
    final String
        publicKey =
    ...
    try {
      ...
      logging.child('FeatureFlags').severe(
