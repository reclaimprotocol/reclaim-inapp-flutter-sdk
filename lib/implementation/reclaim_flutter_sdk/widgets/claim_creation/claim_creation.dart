...
import '../../src/utils/provider_performance_report.dart';
...
import 'state.dart';
import 'status.dart';
import 'trigger_indicator.dart'
    show
        ClaimTriggerIndicatorController;
...
const _claimCreationTimeoutDuration =
    Duration(
        minutes: 15);
...
class ClaimCreationController
  ...
  static ClaimCreationController?
      _lastInstance;
  ...
  static ClaimCreationController?
    ...
    if (listen) {
      return context.dependOnInheritedWidgetOfExactType<ClaimCreationControllerProvider>()?.controller;
    }
    return context
  ...
  void setPublicData(
      Object?
          publicData) {
    value =
        value.copyWith(publicData: publicData);
  }

  void canExpectManyClaims(
      bool
          canExpectManyClaims) {
    value =
        value.copyWith(canExpectManyClaims: canExpectManyClaims);
  }

  void setProviderError(
      Map<String, dynamic>
          error) {
    value =
        value.copyWith(
  ...
  void _setDelegate(
      ClaimCreationUIDelegate?
          delegate) {
    value =
        value.copyWith(delegate: delegate);
  }

  ClaimTriggerIndicatorController get claimTriggerIndicatorController => value
      .delegate!
      ._claimTriggerIndicatorController;
  ...
  void
    ...
    value =
        value.copyWith(
      status:
          ClaimCreationStatus.retryRequested,
      claimsByRequest: const {},
    );
    value =
        value.copyWith(status: ClaimCreationStatus.ready);
  }

  Future<void>
      _onProofGenerationStarted(String sessionId) async {
    final isFirstClaim =
        value.totalCount <= 1;
    if (isFirstClaim) {
  ...
  Future<void>
    ...
    final logger =
        logging.child('ClaimCreationController._onProofGenerated');
    final sessionId =
        proofRequest.sessionId;
    final httpProviderId =
        proofRequest.httpProviderId;
    ...
    if (claimStatus !=
        null) {
      value =
          value.copyWithStatus(claimStatus);
    } else {
      logger.severe(
  ...
  Future<ClaimCreationRequest>
    ...
    final Iterable<ResponseMatch>
        responseMatches =
        request.requestData.responseMatches ?? const [];
    final List<ResponseRedaction>
        responseRedactions =
        [
    ...
    await Future
        .wait(
      List.generate(responseRedactions.length,
          (index) async {
    ...
    logger
      ...
      'responseRedactions.pre':
          request.requestData.responseRedactions?.map((e) => e.toJson()).toList(),
      'responseMatches':
          responseMatches.map((e) => e.toJson()).toList(),
      'responseRedactions':
          responseRedactions.map((e) => e.toJson()).toList(),
    });
    ...
    return request
      ...
      witnessParams:
          params,
      extractedData:
          request.extractedData.copyWith(
        witnessParams: {
          ...params,
          ...request.initialWitnessParams
        },
  ...
  Future<ClaimCreationRequest>
    ...
    final claimStatus =
        value.maybeGet(requestIdentifier);
    if (claimStatus ==
        null) {
      throw StateError(
  ...
  final Map<String,
      ...
      _requestHashByAttestorRequestId =
      {};

  Future<List<CreateClaimOutput>>
      _onCreateClaim(
    ClaimCreationRequest
        proofRequest,
  ) async {
    final requestIdentifier = proofRequest
        .requestData
        .requestIdentifier;
    final log =
        logging.child(
    ...
    final httpProviderId =
        proofRequest.httpProviderId;
    final useSingleRequest =
        proofRequest.useSingleRequest;
    final ownerPrivateKey =
        await getReclaimPrivateKeyOfOwner();

    await _onProofGenerationStarted(
        sessionId);
    ...
    try {
      ...
      final List<Future>
          delegateCallbackFutures =
          [];

      final requestMeasurePerformance =
          MeasurePerformance();
      Iterable<ZKComputePerformanceReport>?
          requestPerformanceReports;

      requestMeasurePerformance.start();

      final attestorRequest =
      ...
      _requestHashByAttestorRequestId[attestorRequest.id] =
          requestIdentifier;
      ...
      final proofs =
          await attestorRequest.response;
  ...
  void _onStep(
      ...
      Map?
          step) {
    final logger =
        logging.child('ClaimCreationController._onStep');
    final status =
        value.maybeGet(requestIdentifier);
    if (status ==
        null) {
    ...
    value =
        value.copyWithStatus(status.createNext(stepInformation: step));
  }

  void
      _onAttestorInitializationProgress(
    ClaimRequestIdentifier
        requestIdentifier,
    double
        progress,
  ...
  final Map<ClaimRequestIdentifier,
          SingleWorkScope<List<CreateClaimOutput>>>
  ...
  SingleWorkScope<List<CreateClaimOutput>>
    ...
    final log =
        logging.child('ClaimCreationController._getSingleWorkScope');
    final ClaimRequestIdentifier
        key =
        request.requestIdentifier;
    final scope =
        _singleWorkScopesByRequest.putIfAbsent(
      key,
      () =>
          SingleWorkScope(),
    );
  ...
  Future<List<CreateClaimOutput>>
    ...
    final completedRequestProofs = value
        .getCompletedRequestBy(requestIdentifier)
        ?.proofs;
    if (completedRequestProofs !=
        null) {
      log.info(
    ...
    log.info(
        'starting claim creation inside controller');

    value =
        value.copyWithStatus(ClaimStatus.create(proofRequest));
    ...
    try {
      final scope =
          _getSingleWorkScope(proofRequest.requestData);

      return scope.runGuarded(() =>
          _onCreateClaim(proofRequest));
  ...
  void
    ...
    value =
      ...
      status:
          ClaimCreationStatus.manualVerificationRequested,
    );
  }
