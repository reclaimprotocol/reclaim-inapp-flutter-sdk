...
class ClaimTriggerIndicatorController
    extends ValueNotifier<
        ClaimTriggerType> {
  ClaimTriggerIndicatorController()
      : super(ClaimTriggerType.none);
  ...
  void
      notifyClaim() {
    value =
        ClaimTriggerType.claim;
  }

  void
      notifyIntelligence() {
    value =
        ClaimTriggerType.intelligence;
  }

  Timer?
      _removeIndicatorTimer;

  void
      _scheduleRemoveIndicator() {
    _removeIndicatorTimer
        ?.cancel();
    _removeIndicatorTimer = Timer(
        Durations.extralong4,
        () {
      // don't use remove because it will do nothing when current value is `error`.
      value =
          ClaimTriggerType.none;
    });
  }
...
class ClaimTriggerIndicator
  ...
  Widget build(
    ...
    return Padding(
      padding:
          EdgeInsets.symmetric(horizontal: mediaQuerySize),
      child:
          Row(
...
class ClaimCreationIndicatorWrapper
    extends StatelessWidget {
  const ClaimCreationIndicatorWrapper(
      {super.key,
      required this.child});
  final Widget
      child;
  ...
  Widget build(
    ...
    return AnimatedBuilder(
      animation:
          controller.claimTriggerIndicatorController,
      builder:
          (context, child) {
