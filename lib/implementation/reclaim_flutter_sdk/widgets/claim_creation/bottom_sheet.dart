...
const _animationDuration =
    Durations
        .short3;
const _animationInCurve =
    Curves
        .easeIn;
const _animationOutCurve =
    Curves
        .easeOut;

class ClaimCreationBottomSheet
    extends StatelessWidget {
  const ClaimCreationBottomSheet(
      {super.key});
  ...
  static Future<void>
    ...
    BuildContext
        context, {
    required ClaimCreationController
        claimCreationController,
    required ClaimCreationUIDelegateOptions
        options,
  }) async {
    if (!context
        .mounted)
  ...
  Widget build(
      BuildContext
          context) {
    final controller =
        ClaimCreationController.of(context);
...
class _LoadingViewState
  ...
  Widget build(
    ...
    final params =
        ParamInfo.fromBuildContext(context);
    final uniqueParamsCount =
        params.uniqueParamsCount;
    ...
    final bool
        isExpanded =
        _isExpanded && isExpandable;

    final Iterable<Widget>
        paramTiles =
        ParamsText.buildTiles(
      context,
      params,
      onlyShowPublicAndInProgressParams:
          !isExpanded,
    );

    late final changingParamTileBuilder =
        ChangingTileBuilder(
      length:
          paramTiles.length,
      builder:
...
class _WaitingForContinuationWidget
    extends StatefulWidget {
  const _WaitingForContinuationWidget();

  @override
  State<_WaitingForContinuationWidget>
      createState() =>
...
class _WaitingForContinuationWidgetState
  ...
  void
    ...
    super
        .initState();
    controller = ClaimCreationController.of(
        context,
        listen: false);
    Future
        .delayed(
    ...
    Future.delayed(
      ...
      if (!mounted)
        return;
      if (!_hasClaimWaitDurationElapsed &&
          !_hasContinued) {
...
class _SuccessViewState
    extends State<
        _SuccessView> {
  late ClaimCreationController
      controller;
  late ClaimCreationUIDelegateOptions?
      options;

  bool get isAutoSubmitEnabled =>
      options?.autoSubmit ==
      true;
  ...
  void
    ...
    controller = ClaimCreationController.of(
        context,
        listen: false);
    options = ClaimCreationUIDelegateOptions.of(
        context,
        listen: false);
  ...
  void
    ...
    controller =
        ClaimCreationController.of(context);
    options =
        ClaimCreationUIDelegateOptions.of(context);
  }
  ...
  void
    ...
    Navigator.of(context)
        .pop();
    final maybeProofs = controller
        .value
        .claims
        .map((e) => e.proofs);
    final proofs =
        maybeProofs.whereType<List<CreateClaimOutput>>();

    assert(proofs.isNotEmpty &&
        proofs.length == maybeProofs.length);
  ...
  double
      get submitViewHeight {
    if (isAutoSubmitEnabled) {
      return _LoadingViewState.getEstimateHeight();
    }
    final submitView = submitViewKey
        .currentContext
        ?.findRenderObject();
    if (submitView
        is RenderBox) {
      final height =
          submitView.size.height;
      return height;
    }
    return 187.0;
  }
  ...
  Widget build(
    ...
    return AnimatedCrossFade(
      key:
          const ValueKey('ClaimSuccessView'),
      duration:
      ...
      secondChild:
          _DataSharedView(height: submitViewHeight, play: isSubmitted),
    );
  }
}

class _DataSharedView
    extends StatefulWidget {
  const _DataSharedView(
      {this.play =
          false,
      this.height});
  ...
  @override
  State<_DataSharedView>
      createState() =>
          _DataSharedViewState();
}

class _DataSharedViewState
    extends State<
        _DataSharedView> {
  final tickKey =
      GlobalKey<DataSharedCheckAnimatedIconState>();
...
class _VerificationCardContent
    extends StatelessWidget {
  const _VerificationCardContent(
      {required this.title});
  final String
      title;
  ...
  Widget build(
    ...
    final controller =
        ClaimCreationController.of(context);
    final providerData = controller
        .value
        .httpProvider;
    final hasVerifiedAtleastOneClaim = controller
        .value
        .hasVerifiedAtleastOneClaim;
...
class _ErrorWidget
  ...
  Widget build(
    ...
    final providerError = controller
        .value
        .providerError;
