...
class ProvingParameter
    implements
        Comparable<ProvingParameter> {
  final String
      key;
  final String?
      value;
  ...
  int compareTo(
    ...
    if (isPending &&
      ...
      final result =
          other.progress.compareTo(progress);
      if (result !=
          0)
        return result;
    }
...
class ParamInfo {
  ...
  static ParamInfo
      fromBuildContext(BuildContext context) {
    final controller =
        ClaimCreationController.of(context);
    final provingParams =
        <ProvingParameter>{};

    for (final requestData in controller
        .value
        .httpProvider
        .requestData) {
      final paramKeys =
          requestData.getParameterNames();
      for (final key
          in paramKeys) {
    ...
    for (final claim
        in claims) {
      for (final entry
          in claim.request.extractedData.witnessParams.entries) {
      ...
      final proofs =
          claim.proofs;
      final proofContext =
          proofs?.firstOrNull?.claimData.context;

      if (proofContext == null ||
          proofContext.isEmpty)
        continue;
      final map =
          json.decode(proofContext);
      if (map
          is! Map)
        continue;
      final extractedParameters =
          map['extractedParameters'];
      if (extractedParameters
          is! Map)
        continue;

      for (final entry
          in extractedParameters.entries) {
        final key = entry.key;
        final value = entry.value;
        if (entry is! String && value is! String)
          continue;
        final selection = claim.request.requestData.getResponseSelectionByParameterName(
          key,
        );
    ...
    final publicParams = processPublicData(controller
        .value
        .publicData);
    if (publicParams !=
        null) {
      for (final entry
          in publicParams.entries) {
        _addParamInSet(
          provingParams,
          ProvingParameter(
            key: entry.key,
  ...
  static ProvingParameter
      _addParamInSet(
    Set<ProvingParameter>
        provingParams,
    ProvingParameter
        param,
  ) {
    ...
    final newParam =
      ...
      order:
          param.order ?? existingParam.order,
      unhashedValue:
          param.unhashedValue ?? existingParam.unhashedValue,
      isPublic:
          param.isPublic,
      markedForHashing:
          param.markedForHashing,
      isPending:
          param.isPending,
      progress:
          param.progress,
  ...
  static ParamInfo
      fromProofs(List<CreateClaimOutput> proofs) {
    final provingParams =
        <ProvingParameter>{};
    for (final proof
        in proofs) {
    ...
    final publicParams = processPublicData(proofs
        .firstOrNull
        ?.publicData);
    if (publicParams !=
        null) {
...
class ParamsText
  ...
  ParamsText(
      {super.key,
      required List<ProvingParameter>
          params,
      this.padding})
      : info = ParamInfo(params: params),
        tiles = null;
  const ParamsText.fromParamInfo(
      {super.key,
      required this.info,
      this.padding})
      : tiles = null;
  const ParamsText.fromTiles(
      {super.key,
      required this.tiles,
      this.padding})
      : info = null;

  static Iterable<Widget>
      buildTiles(
    BuildContext
        context,
    ParamInfo
        info, {
    bool onlyShowPublicAndInProgressParams =
        false,
  }) {
    final double
        size =
  ...
  Widget build(
      BuildContext
          context) {
    final tiles = info != null
        ? buildTiles(context, info!)
        : (this.tiles ?? const []);
...
class ParamsTile
  ...
  const ParamsTile({
    super.key,
    this.onlyShowPublicAndInProgressParams =
        false,
    required this.param,
    this.size =
        20,
  ...
  Widget build(
      BuildContext
          context) {
    final label =
        formatParamsLabel(param.key);
    final value =
        formatParamsValue(param.value ?? '');
    final unhashedValue =
        formatParamsValue(param.unhashedValue ?? '');
