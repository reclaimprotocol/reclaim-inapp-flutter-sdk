...
String
    createManualVerificationInjections(
  bool
      disableRequestReplay,
  InjectionType
      injectionType,
...
typedef ConvertedTemplateResult
    = (
  String template,
  List<
      String> allVars,
  List<
      String> unSubstitutedVars
);
...
ConvertedTemplateResult
  ...
  template = escapeSpecialCharacters(
      template,
      extraEscape:
          extraEscape);
  ...
  final Set<String>
      allVars =
      getTemplateVariables(template).toSet();
  for (final param
      in allVars) {
    if (parameters
        .containsKey(param)) {
      template = interpolateParamValue(
          template,
          param,
          parameters);
    } else {
      ...
      final replacement = matchTypeOverride == MatchType.GREEDY || param.endsWith('GRD')
          ...
          : '(.*?)';
      template =
          template.replaceAll('{{$param}}', replacement);
    }
  }
  return (
    template,
