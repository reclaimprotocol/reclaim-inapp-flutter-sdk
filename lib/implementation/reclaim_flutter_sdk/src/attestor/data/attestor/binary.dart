...
class AttestorBinaryData
    extends AttestorData {
  /// Creates a new [AttestorBinaryData] with a base64 encoded string [value].
  const AttestorBinaryData(
      {required super.value})
      : super(type: 'uint8array');

  factory AttestorBinaryData.fromBytes(
      List<int>
          bytes) {
    final data =
        base64.encode(bytes);
    return AttestorBinaryData(
        value: data);
  }

  factory AttestorBinaryData.fromJson(
      Object?
          json) {
    if (json
        is Map) {
