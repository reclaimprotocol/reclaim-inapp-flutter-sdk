...
Future<void>
    _sendLogEntries(
  List<_BufferLogEntry>
      entries, {
  required SessionIdentity
      fallbackSessionIdentity,
  Uri?
      loggingUrl,
  bool truncateLongerInformation =
      false,
}) async {
  try {
    loggingUrl ??=
    ...
    debugPrintThrottled(
        'Failed to send logs to url $loggingUrl: $e');
    rethrow;
  }
}
...
final _logDateFormat =
    DateFormat(
        'hh:mm:ss aa');

void _onLogsToConsole(
    LogRecord
        record) {
  final formattedTime =
      _logDateFormat.format(record.time);
...
void _onLogRecord(
  ...
  final onRecord = ReclaimOverrides
      .logsConsumer
      ?.onRecord;
  if (onRecord !=
      null) {
    final canHandleLogs = await onRecord(
        record,
        SessionIdentity.latest);
    if (!canHandleLogs) {
...
void _updateAttestorLogLevel(
  ...
  if (isDebug !=
    ...
    _oldIsDebug =
        isDebug;
    final attestorCoreLogLevel = isDebug
        ? 'debug'
        : 'info';
    Attestor
        .instance
        .setAttestorDebugLevel(attestorCoreLogLevel);
  }
}

void _onLogLevelChange(
    Level?
        level) {
  _saveLogLevel(
      level);
  _updateAttestorLogLevel(
      level);
}

const _logLevelPrefKey =
    'reclaim_flutter_sdk#log_level';

Future<Level>
    _loadSavedLevel() async {
  final getLevel = ReclaimOverrides
      .logsConsumer
      ?.levelChangeHandler
      ?.getLevel;
  if (getLevel !=
...
Future<void>
    _saveLogLevel(
        Level? level) async {
  final setLevel = ReclaimOverrides
      .logsConsumer
      ?.levelChangeHandler
      ?.onLevelChanged;
  if (setLevel !=
...
void
    initializeReclaimLogging() async {
  if (_isInitialized)
    return;
  _isInitialized =
      true;
  ...
  logging
      .onLevelChanged
      .listen(_onLogLevelChange);
  logging.level =
      await _loadSavedLevel();
  _updateAttestorLogLevel(
      logging.level);

  final platformDispatcherLogger =
      logging.child('PlatformDispatcher');
  PlatformDispatcher
      .instance
      .onError = (e, s) {
  ...
  final flutterErrorLogger =
      logging.child('FlutterError');
