...
/// pointycastle.
class _DartSecureRandom
    implements
        SecureRandom {
  const _DartSecureRandom();

  Random get dartRandom =>
      Random.secure();
  ...
  BigInt nextBigInteger(
    ...
    final main =
        bytesToUnsignedInt(nextBytes(fullBytes));
    final additional =
        dartRandom.nextInt(1 << remainingBits);
    return main +
        (BigInt.from(additional) << (fullBytes * 8));
  }
...
class NistP256CurveInfo {
  ...
  Map<String,
    ...
    return {
      ...
      'kty':
          'EC',
      'x': x != null
          ? jwkValueCodec.encode(x)
          : null,
      'y': y != null
          ? jwkValueCodec.encode(y)
          : null,
      'crv':
          curveName,
      'd': privateKey.d != null
          ? jwkValueCodec.encode(privateKey.d!)
          : null,
    };
  }
  ...
  ECPrivateKey
      importPrivateKeyFromJwk(Map<String, dynamic> jwk) {
    final d =
        jwkValueCodec.decode(jwk['d']);
    return ECPrivateKey(
        d,
        domainParameters);
  }
  ...
  ECPublicKey
      importPublicKeyFromJwk(Map<String, dynamic> jwk) {
    final x =
        jwkValueCodec.decode(jwk['x']);
    final y =
        jwkValueCodec.decode(jwk['y']);
    final point = domainParameters.curve.createPoint(
        x,
        y);
    return ECPublicKey(
        point,
...
class NistP256ECDSASigner {
  ...
  final String
      kid;
  final NistP256CurveInfo
      curveInfo;
  final AsymmetricKeyPair<
      ECPublicKey,
      ECPrivateKey> keyPair;
  ...
  factory NistP256ECDSASigner.fromJwk(
      Map<String, dynamic>
          jwk) {
    final curveInfo =
        NistP256CurveInfo();
  ...
  Map<String,
          dynamic>
      exportPrivateKey() {
    return curveInfo.exportPrivateKey(
        kid,
        keyPair.privateKey,
        keyPair.publicKey);
  }
  ...
  Map<String,
          dynamic>
      exportPublicKey() {
    return curveInfo.exportPublicKey(
        kid,
        keyPair.publicKey);
  }
  ...
  Uint8List
    ...
    final messageHash = Uint8List.fromList(sha256
        .convert(message)
        .bytes);
    final signer =
        ECDSASigner();
    ...
    signature
        .normalize(curveInfo.domainParameters);
    ...
    setValuesInRange(
        ...
        0,
        32,
        jwkValueEncoder.toBigEndianUint8List(signature.r));
    setValuesInRange(
        signatureBytes,
        32,
        64,
        jwkValueEncoder.toBigEndianUint8List(signature.s));
...
class NistP256ECDSAVerifier {
  ...
  factory NistP256ECDSAVerifier.fromJwk(
      Map<String, dynamic>
          jwk) {
    final curveInfo =
        NistP256CurveInfo();
  ...
  bool verify(
    ...
    final ecSignature =
        ECSignature(
      jwkValueDecoder.toBigIntFromBigEndianBytes(signature.sublist(0,
          32)),
      jwkValueDecoder.toBigIntFromBigEndianBytes(signature.sublist(32,
          64)),
    );
