...
class Jws {
  ...
  factory Jws.fromString(
    ...
    final header =
        JwsHeader.fromJson(json.decode(utf8.decode(urlSafeDecode(parts[0]))));
    final payload =
        urlSafeDecode(parts[1]);
    final signature =
        urlSafeDecode(parts[2]);
    return Jws(
        header: header,
        payload: payload,
        signature: signature);
  ...
  static Uint8List createSignaturePayload(
      JwsHeader
          header,
      Uint8List
          payload) {
...
class ES256Jws
  ...
  factory ES256Jws.create(
    ...
    final signer =
        NistP256ECDSASigner.fromJwk(privateKeyJwk);
    final signaturePayload = Jws.createSignaturePayload(
        header,
        payload);
    final signature =
        signer.sign(signaturePayload);
    return ES256Jws._(
        header: header,
        payload: payload,
        signature: signature);
  ...
  factory ES256Jws.import(
      String
          input,
      Map<String, dynamic>
          publicKeyJwk) {
    final jws =
        Jws.fromString(input);
    final verifier =
        NistP256ECDSAVerifier.fromJwk(publicKeyJwk);
    final signaturePayload = Jws.createSignaturePayload(
        jws.header,
        jws.payload);
