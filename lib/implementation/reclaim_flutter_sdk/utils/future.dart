...
void unawaitedSequence(
    Iterable<Future>
        futures) async {
  final logger =
      logging.child('unawaitedSequence');
  for (int i =
          0;
      i < futures.length;
      i++) {
    try {
      ...
      logger.severe(
        'future[$i] completed with an error',
        e,
        s,
      );
      return;
...
Future<T>
  ...
  final logger =
      logging.child('advancedFutureHandler');
...
Future<void>
    awaitFuturesCompletion(
        Iterable<Future> futures) async {
  if (futures
      .isEmpty)
    return;
  final logger =
      logging.child('awaitFuturesCompletion');
  try {
    await Future.wait(
        futures);
  } catch (e, s) {
    logger.severe(
        'some future completed with an error',
        e,
        s);
  }
}
