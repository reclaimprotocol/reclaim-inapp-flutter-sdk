...
extension _DioResponseExtension<
        T>
    on Future<
        Response<T>> {
  static final _logger =
      logging.child('DioErrorResponse');
  Future<Response<T>>
      logWhenResponseErrors() async {
    try {
      return await this;
...
abstract interface class SessionUpdateHandler {
  ...
  Future<void>
    ...
    final log =
        logging.child('dumpNetworkRequests');
    final dio =
        buildDio();
    ...
    log.finest(
        'Dumped network requests, response: ${response.statusCode}');
  }
  ...
  Future<String> getSignedUploadUrlFromS3ForManualVerificationSessionScreenshot(
    ...
    final url =
        jsonDecode(response.toString())['url'];
  ...
  Future<
      Map<String,
          dynamic>> checkScreenshotWithAI(
      Dio
          dio,
      String
          httpProviderId,
      String
          base64Screenshot) async {
    final dio =
        buildDio();
  ...
  static Future<
      Map<String,
          dynamic>> checkLoggedInStateWithAI(
      String
          url,
      String
          base64Screenshot,
      String
          heuristicSummary) async {
    final dio =
        buildDio();
    final logger =
        logging.child('checkLoggedInStateWithAI');
    final response =
        await dio.post(
  ...
  Future<
      Map<String,
          dynamic>> checkLoggedInStateWithAiV2(
      Dio
          dio,
      String
          url,
  ...
  Future<void>
      createManualVerificationSession(CreateManualVerificationSessionPayload payload) async {
    final dio =
        buildDio();
    dio.options.headers['accept'] =
        '*/*';
    ...
    logging
        .info({
      'msg':
          'Creating manual verification session',
      'request':
          data,
    });
  ...
  Future<
      Map<String,
          dynamic>> getManualVerificationSessionStatus(
      String
          sessionId) async {
    final logger =
        logging.child('getManualVerificationSessionStatus');
    try {
      final dio =
          buildDio();
      final response =
      ...
      if (response.statusCode ==
        ...
        final Map<String, dynamic>? claim = sessionData['aiReceipt']?['claim'] as Map<String, dynamic>?;
        ...
        throw Exception('Failed to get session status. Status code: ${response.statusCode}');
      }
    } catch (error, stackTrace) {
      logger.severe(
          'Error getting manual verification session status',
          error,
          stackTrace);
      rethrow;
    }
...
class _DefaultSessionUpdateHandler
    extends SessionUpdateHandler {
  ...
  Future<void> updateSession(
      ...
      SessionStatus
          status) async {
    final logger =
        logging.child('ReclaimSession.updateSession');
    try {
      final dio =
          buildDio();
      dio.options.headers['Content-Type'] =
  ...
  Future<void>
    ...
    try {
      ...
      logger.severe(
          'Error sending logs to backend server',
          error,
          stackTrace);
    }
  }
...
class _SessionUpdateHandlerImpl
    extends _DefaultSessionUpdateHandler {
  ...
  Future<String>
    ...
    final createSession = ReclaimOverrides
        .session
        ?.createSession;
    if (createSession !=
        null) {
  ...
  Future<void>
    ...
    final updateSession = ReclaimOverrides
        .session
        ?.updateSession;
    if (updateSession !=
        null) {
  ...
  Future<void>
    ...
    final logRecord = ReclaimOverrides
        .session
        ?.logRecord;
    if (logRecord !=
        null) {
  ...
  Future<List<AIFlowDataReceipt>>
    ...
    final log =
        logging.child('extractParamsFromHtml');
    final dio =
        buildDio();
    dio.options.headers['accept'] =
        '*/*';
    ...
    final response = await dio
        ...
        .logWhenResponseErrors();
    log.finest(
        'Extracted params from html, response: ${response.statusCode}');
    final result =
        jsonDecode(response.toString());
    final List<dynamic>
        params =
        result['params'] as List<dynamic>;

    return params
        .map((e) => AIFlowDataReceipt.fromJson(e as Map<String, dynamic>))
        .toList();
  }
