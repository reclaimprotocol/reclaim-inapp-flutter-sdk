...
class ReclaimCacheManager
    extends CacheManager
    with
        ImageCacheManager {
  static const key =
      'libReclaimCachedImageData';

  static final ReclaimCacheManager
      _instance =
      ReclaimCacheManager._();
...
class HttpDioGetResponse
  ...
  Stream<
      List<
          int>> get content =>
      Stream.value(_response.data ??
          const []);

  @override
  int? get contentLength => _response
      .data
      ?.length;
  ...
  DateTime
    ...
    final controlHeader =
        _header(HttpHeaders.cacheControlHeader);
    if (controlHeader !=
        null) {
  ...
  String
    ...
    final contentTypeHeader =
        _header(HttpHeaders.contentTypeHeader);
