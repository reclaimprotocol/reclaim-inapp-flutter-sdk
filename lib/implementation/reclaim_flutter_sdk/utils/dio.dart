...
Dio buildDio() {
  final logger =
      logging.child('buildDio.RetryInterceptor');
  final dio =
      Dio();
  if (!Platform
      .environment
...
class ReclaimDioRetryEvaluator {
  ...
  FutureOr<bool> evaluate(
      ...
      int attempt) {
    final log =
        logging.child('ReclaimDioRetryEvaluator.evaluate');
    bool
        shouldRetry;
    if (error.type == DioExceptionType.unknown ||
      ...
      shouldRetry =
          error.type != DioExceptionType.cancel && error.error is! FormatException;
    }
    currentAttempt =
        attempt;
    return shouldRetry;
