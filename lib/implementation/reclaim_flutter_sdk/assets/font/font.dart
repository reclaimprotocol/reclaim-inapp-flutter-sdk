...
class TargetFontDescription {
  ...
  Future<void>
    ...
    try {
      ...
      ByteData?
          byteData =
          await file_io.loadFontFromDeviceFileSystem(
        name: name,
      ...
      byteData ??=
          await _downloadFontDataFromNetwork(this);

      // Load font in the flutter engine
      await _loadFontInFlutter(name,
          byteData);
    } catch (e, s) {
      logging.child('installFontIfRequired').info(
            'We were unable to load font $name required in reclaim_flutter_sdk package',
            e,
            s,
          );
...
Future<ByteData>
    _downloadFontDataFromNetwork(
  TargetFontDescription
      font,
) async {
  final uri =
      Uri.tryParse(font.url);
  if (uri ==
      null) {
    throw Exception(
        'Invalid fontUrl: ${font.url}');
  }
  ...
  try {
    ...
    throw Exception(
        'Failed to load font with url ${font.url}: $e');
  }
  if (response.statusCode ==
    ...
    if (!_isFileSecure(
      ...
      throw Exception(
        'File from ${font.url} did not match expected length and checksum.',
      );
    }
    ...
    throw Exception(
        'Failed to load font with url: ${font.url}');
  }
}
...
bool
  ...
  final actualFileLength =
      bytes.length;
  final actualFileHash = sha256
      .convert(bytes)
      .toString();
  return font.expectedFileLength ==
          actualFileLength &&
      font.expectedFileHash ==
          actualFileHash;
}
