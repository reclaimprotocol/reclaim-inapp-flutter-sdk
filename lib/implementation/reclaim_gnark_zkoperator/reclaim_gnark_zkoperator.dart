...
final _logger =
    Logger(
        'reclaim_flutter_sdk.reclaim_gnark_zkoperator');
...
class ReclaimZkOperator
  ...
  static Future<ReclaimZkOperator>
      getInstance([
    ...
    if (_cachedInstances[getAssetUrls] ==
        null) {
      _cachedInstances[getAssetUrls] =
          ReclaimZkOperator._(ProverAlgorithmInitializer(getAssetUrls, priority));
    }
    return _cachedInstances[
        getAssetUrls]!;
  }

  static KeyAlgorithmAssetUrls
      defaultProverAlgorithmAssetUrlsProvider(ProverAlgorithmType algorithm) {
    return KeyAlgorithmAssetUrls(
        algorithm.defaultKeyAssetUrl,
        algorithm.defaultR1CSAssetUrl);
  }

  final ProverAlgorithmInitializer
      initializer;
  ...
  Future<String>
    ...
    List<dynamic>
        args, {
    void Function(ProverAlgorithmType?, PerformanceReport)?
        onPerformanceReport,
  }) async {
    final logger =
        Logger('reclaim_flutter_sdk.reclaim_gnark_zkoperator.computeAttestorProof.$fnName');
    ...
    final String
      ...
      switch (fnName) {
        ...
        case 'generateOPRFRequestData':
          ...
          final jsonString = json.encode(_replaceBase64Json({
            'data': data,
    ...
    return _reformatJsonStringForRPC(
        response);
  }

  @override
  Future<String> groth16Prove(
      Uint8List
          bytes,
      {OnProofPerformanceReportCallback?
          onPerformanceReport}) async {
    final proveWorkerFuture =
        _proveWorkerFuture ??= _ProveWorker.spawn();
    final worker =
        await proveWorkerFuture;
    return worker.prove(
        bytes,
        onPerformanceReport: onPerformanceReport);
  }

  Future<_TOPRFFinalizeWorker>?
      _toprfFinalizeWorkerFuture;

  @override
  Future<String>
      finaliseOPRF(Uint8List bytes) async {
    final workerFuture =
  ...
  Future<_GenerateOPRFRequestDataWorker>?
      _generateOPRFRequestDataWorkerFuture;

  @override
  Future<String>
      generateOPRFRequestData(Uint8List bytes) async {
    final workerFuture =
        _generateOPRFRequestDataWorkerFuture ??= _GenerateOPRFRequestDataWorker.spawn();
    final worker =
        await workerFuture;
    return worker
        .generateOPRFRequestData(bytes);
  }

  @override
  Future<void>
      close() async {
    final proverWorkerFuture =
        _proveWorkerFuture;
    if (proverWorkerFuture !=
        null) {
