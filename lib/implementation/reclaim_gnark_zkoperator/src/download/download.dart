...
final _buildClientLogger =
    Logger(
        'reclaim_flutter_sdk.reclaim_gnark_zkoperator._buildClient');

http.Client
    _buildClientWithDefaultCaching() {
  _buildClientLogger
      .config('Building client with default caching');
  if (Platform
      .isAndroid) {
    return cronet.CronetClient
        .fromCronetEngine(
    ...
    config.discretionary =
        true;
    config.allowsCellularAccess =
        true;
    config.allowsConstrainedNetworkAccess =
        true;
...
http.Client
    _buildClient(
        [String? cacheDirName]) {
  final cacheDir =
      _getCacheDir(cacheDirName);
  _buildClientLogger
      .config('Building client with cache dir $cacheDir');
  try {
    if (Platform
        .isAndroid) {
      ...
      config.discretionary =
          true;
      config.cache =
          cupertino.URLCache.withCapacity(
...
class _RetryableHttpException
    extends http
    .ClientException {
  _RetryableHttpException(
      super.message,
      [super.uri]);
}

class _EmptyResponseException
    extends http
    .ClientException {
  _EmptyResponseException(
      String
          message,
      [Uri?
          uri])
...
extension _ReadUnstreamed
  ...
  Future<Uint8List>
      readUnstreamed(Uri uri) async {
    final request = http.Request(
        'GET',
        uri);
    final streamedResponse =
        await send(request);
    final bodyBytes = await streamedResponse
        .stream
        .toBytes();
    final statusCode =
    ...
    if (statusCode >=
        500) {
      throw _RetryableHttpException(_createMessage(streamedResponse, uri),
          uri);
    }
    if (bodyBytes
        .isEmpty) {
      throw _EmptyResponseException(_createMessage(streamedResponse, uri),
          uri);
    }
    return bodyBytes;
  }
...
final _downloadWithHttpLogger =
    Logger(
        'reclaim_flutter_sdk.reclaim_gnark_zkoperator.downloadWithHttp');
