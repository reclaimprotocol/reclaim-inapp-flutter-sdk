...
class KeyAlgorithmAssetUrls {
  ...
  const KeyAlgorithmAssetUrls(
      this.keyAssetUrl,
      this.r1csAssetUrl);
}

typedef ProverAlgorithmAssetUrlsProvider
    = KeyAlgorithmAssetUrls
        Function(ProverAlgorithmType algorithm);
...
final _algorithmInitializerFutureCache =
    <ProverAlgorithmType,
        Future<bool>?>{};
final _initAlgorithmWorkerFuture =
    InitAlgorithmWorker.spawn(
        'prover_http_cache');

final _initializerLog =
    Logger(
        'reclaim_flutter_sdk.reclaim_gnark_zkoperator.initializer');

final _initializedAlgorithms =
    ValueNotifier<Set<ProverAlgorithmType>>(
        {});

/// For letting consumers get some information about the initialization status.
ValueListenable<
        Set<ProverAlgorithmType>>
    get initializedAlgorithmsNotifier {
...
Future<bool> _initialize(
    ...
    ProverAlgorithmAssetUrlsProvider
        getAssetUrls) async {
  if (_algorithmInitializerFutureCache[algorithm] ==
      null) {
    final completer =
        Completer<bool>();
    _algorithmInitializerFutureCache[algorithm] =
        completer.future;

    try {
      final worker =
          await _initAlgorithmWorkerFuture;
      final assetUrls =
          getAssetUrls(algorithm);
      final stopwatch =
          Stopwatch();
      stopwatch.start();
      ...
      completer.completeError(e,
          s);
      final algorithmFuture =
          completer.future;
      _algorithmInitializerFutureCache[algorithm] =
          null;
      _initializerLog.severe(
          'Error initializing algorithm $algorithm',
          e,
          s);
      return algorithmFuture;
    }
  }
  return _algorithmInitializerFutureCache[
      algorithm]!;
}

bool
    _hasAllAlgorithmsInitialized =
    false;
...
class ProverAlgorithmInitializer {
  final ProverAlgorithmAssetUrlsProvider
      getAssetUrls;
  final ProverAlgorithmInitializationPriority
      downloadPriority;
  ...
  ProverAlgorithmInitializer(
    ...
    unawaited(
        _startAllInitialized());
  }

  Future<bool>
      ensureInitialized(ProverAlgorithmType algorithm) {
    return _initialize(
        algorithm,
        getAssetUrls);
  }
  ...
  Future<void>
      _startAllInitialized() async {
    if (_hasAllAlgorithmsInitialized)
      return;
    final stopwatch =
        Stopwatch();
    _initializerLog
        .info('Initializing all gnark zk operator algorithms');
    stopwatch
        .start();
    ...
    stopwatch
        .stop();
    _initializerLog
        .info('All gnark zk operator algorithms initialized in ${stopwatch.elapsed}');
  }

  /// Initializes the prover by loading the necessary algorithms asynchronously.
  Future<void>
      _initializeAllAlgorithms() async {
    ...
    await Future
      ...
      [
        ...
        if (canDownloadChachaOprfWithNonOprf)
